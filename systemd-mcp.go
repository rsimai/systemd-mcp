...
func main() {
	...
	server.AddTools(mcp.NewServerTool("list_systemd_host_units_by_state",
		...
		mcp.Input(
			mcp.Property("states", mcp.Description("List of the states")),
		)),
	)
	server.AddTools(mcp.NewServerTool("list_systemd_user_units_by_name",
		...
		mcp.Input(
			mcp.Property("names", mcp.Description("List units with the given by it's exact name. Regexp can be used as e.g. foo* expands to foo.service")),
		)),
	)
	if *httpAddr != "" {
